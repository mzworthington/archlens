...
That script also bootstraps [Waykit](https://github.com/mzworthington/waykit) when needed (sibling checkout or clone). Skip with `SKIP_LIFECYCLE_KIT=1`. Check the handshake with `wk align .`. Project MCP is the kit `default` profile (`.cursor/mcp.json`). For live Cloudflare work, `wk mcp cloudflare-ops --project` for that session, then restore `wk mcp default --project`.
...
**Product analytics:** Canvas reads `POSTHOG_TOKEN` (PostHog project API key). Put it in the environment or `app/packages/canvas/.env.development.local`. Production builds on `main` bake `secrets.POSTHOG_TOKEN`. Missing token: the consent prompt still appears locally so you can opt in or decline; capture stays off until a token is present. In `pnpm dev` the console logs that events will be missed. Ingest defaults to the reverse proxy `https://a.mzworthington.co.uk` (`POSTHOG_HOST` to override). After opt-in the SDK uses cookie persistence (`localStorage+cookie`). A decline stores `archlens.analytics.consent=denied` and never inits PostHog. Privacy notice: `/privacy`. Self-driving (scouts, GitHub) is configured in the PostHog app, not this repo.
