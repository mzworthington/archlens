...
| Publish blueprint catalog | [`publish-blueprint-catalog.yml`](../../.github/workflows/publish-blueprint-catalog.yml) | Assemble declared ArchLens context → scan → fragment → compose `estates/samples/`        | Cron daily **05:00 UTC**; `workflow_dispatch`                  |
...
| `archlens`              | Publish blueprint catalog             | Declared seed from [`scripts/declared-contexts/archlens.json`](../../scripts/declared-contexts/archlens.json); scan hydrates systems |
| `{id}` (e.g. backstage) | Publish demo catalog (per matrix leg) | `contextDeclaration` in [`scripts/blueprint-sample-repos.json`](../../scripts/blueprint-sample-repos.json) assembled before scan     |

**Declared context seeds (ADR-0015):** pipelines run [`scripts/assemble-context-seed.mjs`](../../scripts/assemble-context-seed.mjs) to write a sparse `level: context` YAML (personas, system anchors, third-parties) under the scan output before `archlens scan`. Scan hydrates discoveries into that seed. ArchLens personas: Architect, Contributor, Operator; external: Cloudflare R2 Catalog bucket.
