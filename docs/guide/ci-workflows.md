...
2. **Safety net** — [`compose-catalog.yml`](../../.github/workflows/compose-catalog.yml) re-composes `estates/samples/` hourly (`--allow-empty` so an empty fragment set does not fail). When the composed content hash already matches `latest.revision`, compose is a **no-op** (no snapshot re-upload). Resilience is layered: R2 adapter retries → compose retries on transient storage errors / CAS → workflow retries the whole command up to 3 times.
