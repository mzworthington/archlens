...
| ------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| CI & Deployment Pipeline  | [`ci.yml`](../../.github/workflows/ci.yml)                                               | Quality gate, canvas build; on `main` deploys Pages + collab, smokes SHA, rolls back on failure; `merge-gate` aggregates required checks | `push` / `pull_request` → `main`; `workflow_dispatch`          |
...
| Pulumi Cloudflare         | [`pulumi-cloudflare.yml`](../../.github/workflows/pulumi-cloudflare.yml)                 | Thin caller → edge-dns reusable workflow; preview then **pulumi-prod** gated `up`                                                        | `infra/cloudflare/**` on PR / `main`; `workflow_dispatch`      |
| CodeQL Analysis           | [`codeql.yml`](../../.github/workflows/codeql.yml)                                       | CodeQL (JS/TS incl. Pulumi TS, Go ChaosLens, GitHub Actions) → GitHub Security                                                           | `push` / `pull_request` → `main`; cron daily **12:00 UTC**     |
...
Make **merge-gate** the required status check on `main` (PR and push). Smoke is skipped on PRs; the gate treats that as success and requires smoke only when `main` deploys.
...
2. **Safety net** - [`compose-catalog.yml`](../../.github/workflows/compose-catalog.yml) re-composes `estates/samples/` hourly (`--allow-empty` so an empty fragment set does not fail). When the composed content hash already matches `latest.revision`, compose is a **no-op** (no snapshot re-upload). The workflow stays a thin caller; transient R2 `InternalError` / 5xx handling and CAS backoff live in the CLI. Canvas consume uses the HTTP adapter’s matching 8-attempt 5xx budget. Concurrent publish/consume under injected 5xx is covered by `@archlens/storage` `catalogPublishConsumeLoad.test.ts` (not a live R2 soak).
...
Compose jobs share the GitHub Actions concurrency group `samples-estate-compose` so only one samples-estate stitch runs at a time. Compose reloads fragments and backs off on CAS conflicts and transient R2 errors (`--max-retries`, default 8). Fragment load reads every staged manifest but fetches YAML bodies only for the freshest run per `fragmentKey`.
