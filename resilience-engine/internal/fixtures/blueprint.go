...
// finds the ArchLens repo root containing blueprints/chaoslens-stress.
func FindRepoRoot(start string) (string, error) {
	...
	for {
		candidate := filepath.Join(dir, "blueprints", "chaoslens-stress")
		if info, err := os.Stat(candidate); err == nil && info.IsDir() {
			return dir, nil
		}
		parent := filepath.Dir(dir)
		if parent == dir {
			return "", fmt.Errorf("repo root with blueprints/chaoslens-stress not found from %q", start)
		}
		dir = parent
	}
}
