...
const designerRoot = path.join(path.dirname(fileURLToPath(import.meta.url)), '..');
const manifestPath = path.join(designerRoot, 'lighthouse-report', 'manifest.json');
const assertionsPath = path.join(designerRoot, '.lighthouseci', 'assertion-results.json');
...
function buildSummary() {
  ...
  if (failures.length > 0) {
    ...
    for (const assertion of failures) {
      ...
      const actual =
        assertion.actual == null ? '—' : String(Math.round(assertion.actual * 100));
