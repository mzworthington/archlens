...
Interactive C4 canvas over **BlueprintSpec** diagrams: local folder authoring, YAML that stays in sync with the canvas and optional read-only catalogs published from CI.
...
- **Published catalogs:** **Load sandbox** uses the remote BlueprintSpec corpus on Cloudflare R2 when `VITE_REMOTE_CATALOG_BASE_URL` is set (default in `.env.development` → `https://blueprints.archlens.dev/estates/samples/`). Unset or empty falls back to mirrored `samples/` under `/bundled-blueprints/` (PR/offline). See [`.env.example`](./.env.example).
- **Share-link collab:** Production builds set `VITE_COLLAB_WS_URL=wss://collab.archlens.dev`. Local default is BroadcastChannel; point at a local Worker with `VITE_COLLAB_WS_URL=ws://127.0.0.1:8787`.
- **PostHog:** Optional. Set `POSTHOG_TOKEN` (project `phc_` key). Unset skips init. Events go to `https://a.mzworthington.co.uk` unless `POSTHOG_HOST` is set. See [`.env.example`](./.env.example).
...
To compile the production assets (placed in `app/packages/canvas/dist/`), run from the `/app` directory:

```bash
pnpm --filter @archlens/canvas build
```

To preview the compiled production build locally:

```bash
pnpm --filter @archlens/canvas preview
```
...
```bash
pnpm --filter @archlens/canvas test:e2e
```
...
```bash
pnpm --filter @archlens/canvas test
```
